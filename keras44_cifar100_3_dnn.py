...
x_train = x_train.reshape(50000,3072).astype('float32')/255. #데이터는 0~255여서 /255를 해주면 0~1로 좁혀진다 =>전처리 float-실수형  
x_test = x_test.reshape(10000,3072).astype('float32')/255. #이것도 가능 
...
model.fit(x_train, y_train, epochs=100, verbose=1, batch_size=100, callbacks=[early_stopping])
