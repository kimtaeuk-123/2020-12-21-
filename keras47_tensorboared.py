...
> dir/w
   [ train ], [validation]  폴더가 보인다 

> tensorboard --logdir=.

021-01-11 17:59:26.677191: I tensorflow/stream_executor/platform/default/dso_loader.cc:48] Successfully opened dynamic library cudart64_101.dll
...
사이트에서
127.0.0.1:6006쳐서 들어가기

*새로 만들면 엉킬수 있으니 기존 폴더는 삭제해주고 다시 만들어줘야 한다.
'''
...
modelpath = '../data/modelCheckpoint/k45_mnist_{epochs:02d}-{val_loss:.4f}.hdf5' #. <-지금 study 폴더, 02d -정수형 , f= float 

early_stopping = EarlyStopping(monitor='val_loss', patience=5)

model_checkpoint = ModelCheckpoint(filepath='', monitor = 'val_loss', save_best_only=True, mode='auto')

tensor_board = TensorBoard(log_dir='graph', histogram_freq=0, write_graph=True, write_images=True) #./graph 도 됨 

model.compile(loss='categorical_crossentropy', optimizer='adam', metrics='acc')
hist = model.fit(x_train, y_train, epochs=10, verbose=1,validation_split=0.2, batch_size=10, callbacks=[early_stopping, model_checkpoint, tensor_board])
